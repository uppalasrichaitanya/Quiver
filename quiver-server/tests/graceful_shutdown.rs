...
fn start_server(data: &Path, wal: &Path, address: SocketAddr, log_path: &Path) -> ServerProcess {
    // tracing_subscriber's default fmt writer is stdout.
...
fn graph_snapshot_path(data: &Path) -> PathBuf {
    let mut s = std::ffi::OsString::from(data.as_os_str());
    s.push(".graph");
    PathBuf::from(s)
}

#[test]
fn graceful_shutdown_flushes_snapshot_and_reopen_skips_rebuild() {
    ...
    // First run: insert a vector, then request a graceful shutdown.
    let mut first = start_server(&data, &wal, address, &first_log);
    ...
    // The server must exit on its own and leave a graph snapshot behind.
    ...
    assert!(
        graph_snapshot_path(&data).exists(),
        "graceful shutdown must write the graph snapshot"
    );

    // Second run: reopen must load the snapshot instead of rebuilding.
    ...
    assert!(
        ...
        "second run should load the snapshot, got log: {log}"
    );
    assert!(
        !log.contains("Rebuilding HNSW graph"),
